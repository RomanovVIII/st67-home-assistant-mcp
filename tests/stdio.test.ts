...
it('exposes only the three tools and a secret-free unconfigured status',async()=>{
  const {client}=await connect();
  expect((await client.listTools()).tools.map(t=>t.name).sort()).toEqual(['ha_rest','ha_status','ha_ws']);
